...
if (fs.existsSync(YOURSCRAPINGAPP_EXE_PATH)) {
    console.log(`[MCP Server Debug] EXE Path EXISTS: ${YOURSCRAPINGAPP_EXE_PATH}`);
}
else {
    console.error(`[MCP Server Debug] EXE Path DOES NOT EXIST: ${YOURSCRAPINGAPP_EXE_PATH}`);
}
if (fs.existsSync(YOURSCRAPINGAPP_EXE_DIR)) {
    ...
    try {
        ...
        console.error(`[MCP Server Debug] Could not read EXE Dir: ${YOURSCRAPINGAPP_EXE_DIR}`, e);
    }
}
else {
    console.error(`[MCP Server Debug] EXE Dir DOES NOT EXIST: ${YOURSCRAPINGAPP_EXE_DIR}`);
}
...
async function runYourScrapingAppTool(taskName, params) {
    ...
    try {
        ...
        if (execError.code !== undefined)
            detailedErrorMsg += ` Exit code: ${execError.code}.`;
...
async function startHttpServer() {
    ...
    app.post('/mcp', async (req, res) => {
        ...
        try {
            ...
            if (!res.headersSent) {
                res.status(500).json({ jsonrpc: '2.0', error: { code: -32603, message: 'Internal server error during MCP processing.' }, id: requestId });
            }
            else {
                console.error("[HTTP Server] Headers already sent, cannot send 500 error response.");
            }
            if (transport)
                transport.close();
    ...
    const shutdown = (signal) => {
        ...
        httpServer.close((err) => {
            if (err) {
                console.error("[System] HTTP server shutdown error:", err);
                process.exit(1);
            }
            else {
